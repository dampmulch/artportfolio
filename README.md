...
- `assets/shaders/crazy-dmt-bg.js` — CrazyDMT glitch engine (third-party kit, do not modify; attached via `CrazyDMT.attach`). Ships three presets; the lottery runs `ModulateHueCrazyDMT` (slow hue-breathing with a feedback trail).
...
Each page load runs a 3-way lottery for the background: the video-texture shader (default look: "Tokenized · Leaf lines" over `abstract3.mp4`), Signal Wash in Liquidline, or CrazyDMT in ModulateHueCrazyDMT over `abstract5.mp4`. Append `?bg=<mode>` to force one (e.g. `?bg=CrazyDMT`). A minimal loader covers the page until the winning engine has painted real frames (4s safety timeout).
